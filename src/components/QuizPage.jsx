...
const QuizPage = ({ onComplete }) => {
  ...
  const assignDish = () => {
    ...
    // Push menu items into weightedItems array multiple times based on their price
    for (const [item, price] of Object.entries(menuItems)) {
      const weight = price / 10; // e.g., 40 = 4 weights, 100 = 10 weights
  ...
  const handleAnswer = (optionIndex) => {
    ...
    if (step < questions.length - 1) {
      ...
      const dish = assignDish(); // Assign a random weighted dish
      onComplete(dish);
      navigate("/result");
    }
  };
